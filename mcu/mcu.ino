...
#define servo_angle_open34 155       // Angle to open the servos 3 & 4
...
#define PCMD_SHOW_STATISTICS "sts"       // Show the statistics of the system
...
void setup() {
  ...
  platform.attach(pin_servo_platform);
...
void loop() {
  if (Serial.available() > 0) {
    ...
    if (command.equalsIgnoreCase(PCMD_GET_PROTOCOL_VERSION)) {
      ...
      Serial.print(
          encodeMessage(is_err_led_on ? PRES_ERR_LED_ON : PRES_ERR_LED_OFF));
      ...
      Serial.print(encodeMessage(platform_open ? PRES_PLATFORM_OPEN
                                               : PRES_PLATFORM_CLOSED));
    } else if (command.equalsIgnoreCase(PCMD_PLATFORM_OPEN)) {
      platform.write(servo_angle_open_platform);
      platform_open = true;
